...
function ComplaintDetails() {
  ...
  const getImageUrl = (imagePath) => {
    ...
    // 1. Return direct link if it's already an absolute URL
    if (imagePath.startsWith("http://") || imagePath.startsWith("https://")) {
      return imagePath;
    }

    // 2. Extract only the filename (handles Windows backslashes & Linux forward slashes)
    const filename = imagePath.split(/[/\\]/).pop();

    // 3. Append to backend server base route
    ...
    <Layout>
      <div className="complaint-details-container">
        ...
        <div className="section">
          ...
          <p>
            <strong>State:</strong> {complaint.address?.state || "-"}
          </p>
          <p>
            <strong>District:</strong> {complaint.address?.district || "-"}
          </p>
          <p>
            <strong>City:</strong> {complaint.address?.city || "-"}
          </p>
          <p>
            <strong>Area:</strong> {complaint.address?.area || "-"}
          </p>
          <p>
            <strong>Landmark:</strong> {complaint.address?.landmark || "-"}
          </p>
          <p>
            <strong>Pincode:</strong> {complaint.address?.pincode || "-"}
          </p>
        </div>
          ...
          complaint.assignmentHistory.length === 0 ? (
            ...
            complaint.assignmentHistory.map((item) => (
              <div className="history-card" key={item._id}>
                <p>
                  <strong>Officer:</strong> {item.officer?.name || "N/A"}
                </p>
                <p>
